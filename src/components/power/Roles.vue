            ...
            <el-table :data="roleList" border stripe>
                ...
                <el-table-column type="expand">
                    <template slot-scope="scope">
                        <el-row :class="['vcenter', 'bdbottom', i1 === 0 ? 'bdtop' : '']" v-for="(item1, i1) in scope.row.childrens" :key="item1.id">
                            <!-- 渲染一级权限 -->
                            <el-col :span="5">
                                <el-tag
                                    closable 
                                    @close="removeAuthById(scope.row, item2.id)">{{ item1.authName }}</el-tag>
                                <i class="el-icon-caret-right"></i>
                            </el-col>
                            <!-- 渲染二级权限 -->
                            <el-col :span="19">
                                <!-- 通过for 循环嵌套渲染 二级权限 -->
                                <el-row :class="['vcenter', i2 ===0 ? '' : 'bdtop']" v-for="(item2, i2) in item1.childrens" :key="item2.id">
                                    <el-col :span="6">
                                        <el-tag type="success"
                                            closable 
                                            @close="removeAuthById(scope.row, item2.id)">{{ item2.authName }}</el-tag>
                                        <i class="el-icon-caret-right"></i>
                                    </el-col>
                                    <el-col :span="18">
                                        <el-tag 
                                            ...
                                            type="warning"
                                            closable 
                                            @close="removeAuthById(scope.row, item3.id)">{{ item3.authName }}</el-tag>
                                    </el-col>
                                </el-row>
                            </el-col>
                        </el-row>
    ...
    methods: {
        ...
        async removeAuthById(role, authId) {
            ...
            if(res !== 'confirm') {
                ...
                // console.log('确认删除')
                const { data: res } = await this.$http.delete(`role/${role.id}/auth/${authId}`)
                if(res.code === 200) {
        ...
        async allotAuths() {
            ...
            // console.log(idStr)
            const { data: res } = await this.$http.post(`role/${this.roleId}/auth`, { rids: idStr })
            if(res.code === 200) {
