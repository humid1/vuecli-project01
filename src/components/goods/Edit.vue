        ...
        >
          <el-tab-pane label="基本信息" name="0">
            ...
            <el-form-item label="商品数量" prop="gstockQuantity">
              <el-input
                v-model="editForm.gstockQuantity"
                type="number"
              ></el-input>
            </el-form-item>
          ...
          <el-tab-pane label="商品内容" name="4">
            ...
            <!-- 添加商品的按钮 -->
            <el-button type="primary" class="addBtn" @click="update"
              >更新商品</el-button
            >
          </el-tab-pane>
        </el-tabs>
  ...
  data() {
    return {
      ...
      editFormRules: {
        ...
        gCat: [{ required: true, message: '请选择商品分类', trigger: 'blur' }]
      },
      // 商品分类列表
      cateList: [],
      // 级联选择器的所选值
  ...
  methods: {
    async getGoods() {
      ...
      if (res.code === 200) {
        ...
        item.attrs.attrVal = item.attrs.attrVal
          ? item.attrs.attrVal.split(' ')
          : []
        // 图片回显的参数
    ...
    handleRemove(file, fileList) {
      ...
      if (file.response) {
        ...
        // 获取文件名称
        const filePath = file.url.substring(
          file.url.indexOf('http://127.0.0.1:8085/')
        )
        // 获取索引值
        const index = this.editForm.pics.findIndex(i => (i.pic = filePath))
        this.editForm.pics.splice(index, 1)
      }
    },
    // 文件上传成功时的钩子
